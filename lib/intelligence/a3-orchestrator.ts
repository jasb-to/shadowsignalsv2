...
export interface A3Context { derivatives?:DerivativesSnapshot|null; flow?:FlowSnapshot|null; historical?:HistoricalIntelligence|null; breadthScore?:number|null; cycleScore?:number|null; strategicScore?:number|null; flowScore?:number|null; transitionScore?:number|null; cycle?:CycleResult|null; divergence?:DivergenceResult|null; transition?:RegimeTransition|null }
export interface A3Intelligence extends A3EngineResult { context:{derivatives:DerivativesSnapshot|null;flow:FlowSnapshot|null;historical:HistoricalIntelligence|null;cycle:CycleResult|null;divergence:DivergenceResult|null;transition:RegimeTransition|null}; dataQuality:{available:number;total:number;completeness:number}; evidence:string[] }
...
export function buildA3Intelligence(market:MarketState,ctx:A3Context={}):A3Intelligence {
 ...
 const parts=[tactical.score*.30]
 if(divergence)parts.push(divergence.score*.15)
 if(valid(ctx.flowScore))parts.push(((ctx.flowScore as number)-50)*.15)
 if(valid(ctx.breadthScore))parts.push(((ctx.breadthScore as number)-50)*.10)
 ...
 if(valid(ctx.transitionScore))parts.push(((ctx.transitionScore as number)-50)*.10)
 const contextual=clamp(parts.reduce((a,b)=>a+b,0));const contextWeight=.45*Math.min(1,parts.length/7)
 ...
 const checks=[market,market.indicators,derivative,flow,ctx.historical,divergence,cycle,transition,ctx.breadthScore,ctx.cycleScore,ctx.strategicScore,ctx.flowScore];const available=checks.filter(x=>x!=null).length;const total=checks.length;const completeness=Math.round(available/total*100)
 const confidenceBase=base.confidence*.50+tactical.confidence*.25+(divergence?Math.min(95,50+Math.abs(divergence.score)*.4)*.08:0)+(cycle?Math.min(95,50+Math.abs(cycle.score-50)*.5)*.08:0)+(transition?transition.confidence*.09:0);const confidence=Math.round(Math.max(40,Math.min(94,confidenceBase*(.72+.28*completeness/100))))
 ...
 return {...base,score:mergedScore,signal,confidence,confluence:Math.round((base.confluence+tactical.confidence+(divergence?Math.abs(divergence.score):0)+(ctx.flowScore??50))/4),context:{derivatives:derivative,flow,historical:ctx.historical??null,cycle,divergence,transition},dataQuality:{available,total,completeness},evidence}
}
