...
import { calibrateConfidence, type CalibratedConfidence } from "@/lib/intelligence/calibrated-confidence"
import type { CalibrationBin } from "@/lib/intelligence/calibration"
...
export interface A3Context { derivatives?:DerivativesSnapshot|null;derivativesResult?:DerivativesResult|null;flow?:FlowSnapshot|null;historical?:HistoricalIntelligence|null;crossAsset?:CrossAssetResult|null;breadthScore?:number|null;cycleScore?:number|null;strategicScore?:number|null;flowScore?:number|null;transitionScore?:number|null;cycle?:CycleResult|null;divergence?:DivergenceResult|null;transition?:RegimeTransition|null;calibrationBins?:CalibrationBin[]|null }
export interface A3Intelligence extends Omit<A3EngineResult,"dataQuality"> { context:{derivatives:DerivativesSnapshot|null;derivativesResult:DerivativesResult|null;flow:FlowSnapshot|null;historical:HistoricalIntelligence|null;crossAsset:CrossAssetResult|null;cycle:CycleResult|null;divergence:DivergenceResult|null;transition:RegimeTransition|null}; dataQuality:{available:number;total:number;completeness:number}; evidence:string[]; calibratedConfidence:CalibratedConfidence }
...
export function buildA3Intelligence(market:MarketState,ctx:A3Context={}):A3Intelligence {
 ...
 const confidenceBase=base.confidence*.42+tactical.confidence*.22+(derivativeResult?Math.min(95,50+Math.abs(derivativeResult.score)*.35)*.08:0)+(crossAsset?Math.min(95,50+Math.abs(crossAsset.score)*.35)*.07:0)+(divergence?Math.min(95,50+Math.abs(divergence.score)*.4)*.07:0)+(cycle?Math.min(95,50+Math.abs(cycle.score-50)*.5)*.07:0)+(transition?transition.confidence*.07:0);const rawConfidence=Math.round(Math.max(40,Math.min(94,confidenceBase*(.72+.28*completeness/100))))
 const calibratedConfidence=calibrateConfidence(rawConfidence,ctx.calibrationBins??[]);const confidence=Math.round(Math.max(40,Math.min(94,calibratedConfidence.calibrated)))
 const evidence=[...base.factors.filter((f:A3Factor)=>f.state!=="neutral").map((f:A3Factor)=>`${f.name}: ${f.state}`),...tactical.factors];if(derivativeResult)evidence.push(...derivativeResult.signals);if(crossAsset)evidence.push(...crossAsset.signals);if(divergence)evidence.push(...divergence.signals);if(cycle)evidence.push(...cycle.signals.map(s=>`Cycle: ${s}`));if(transition)evidence.push(...transition.signals.map(s=>`Transition: ${s}`));if(derivative?.fundingRate!=null)evidence.push(`Funding ${derivative.fundingRate>0?"positive":"negative"}`);if(derivative?.longShortRatio!=null)evidence.push(`Long/short ${derivative.longShortRatio.toFixed(2)}`);if(flow?.volumeChangePct!=null)evidence.push(`Volume ${flow.volumeChangePct>=0?"up":"down"} ${Math.abs(flow.volumeChangePct).toFixed(1)}%`);if(ctx.historical)evidence.push(`Historical analogue: ${ctx.historical.matches} matches`);if(valid(ctx.breadthScore))evidence.push(`Breadth ${Math.round(ctx.breadthScore as number)}`);if(valid(ctx.strategicScore))evidence.push(`Strategic ${Math.round(ctx.strategicScore as number)}`);if(calibratedConfidence.method==="empirical")evidence.push(`Confidence calibrated from ${calibratedConfidence.sampleSize} resolved forecasts`)
 ...
 return {...base,score:mergedScore,signal,confidence,confluence,context:{derivatives:derivative,derivativesResult:derivativeResult,flow,historical:ctx.historical??null,crossAsset,cycle,divergence,transition},dataQuality:{available,total,completeness},evidence,calibratedConfidence}
}
